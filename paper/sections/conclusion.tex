...
Consistent with the artifact-first stance described in the introduction, this version of the paper does \emph{not} report headline numeric results; the empty cells in \S\ref{sec:experiments} are forward references to outputs that the evaluation harness in \texttt{evaluation\_cognitive/} will produce once its reproducibility checklist is fully ticked. The ablation matrix, metric definitions, and baseline set are fixed in the configs today, so the specific hypotheses we commit to refuting or confirming in a later version are already part of the public record. We believe this positioning is the honest one while the cognitive evaluation harness is still being built.
